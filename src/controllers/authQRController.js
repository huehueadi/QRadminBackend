...
export const redirectQrCode = async (req, res) => {
  ...
  try {
    ...
    // Find the QR code document and extract the URL
    const qrCodeData = await Qr.findOne({ qrCodeId });
    if (!qrCodeData) {
      return res.status(404).json({ message: "QR code not found" });
    }
    ...
    const analyticsData = new Analytics({
      ...
      ip_address: ip,
      location: location,
      redirection_link: qrCodeData.url, // Save the URL being redirected to
    });

    await analyticsData.save();

    // Perform the redirection using the extracted URL
    res.redirect(qrCodeData.url);
